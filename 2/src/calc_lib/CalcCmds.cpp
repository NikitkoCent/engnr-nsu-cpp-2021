...
void abs_cmd::execute(CalcContext &_calc)
{
    ...
    else
    {
        ...
        try
        {
            item > 0 ? _calc.m_stack.push(item) : _calc.m_stack.push(-item);
        }
...
std::vector<std::string> split(const std::string &s)
{
    ...
    while ((pos_end = trimmed.find(delimiter, pos_start)) != std::string::npos)
    {
        token = trimmed.substr(pos_start, pos_end - pos_start);
        pos_start = pos_end + delim_len;
        res.push_back(token);
    }

    res.push_back(trimmed.substr(pos_start));
    return res;
}